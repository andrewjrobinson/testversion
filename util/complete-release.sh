...
if [ "$?" != "0" ]; then
    echo "git checkout $PROD_BRANCH:  Failed (with code $?), exiting"
    exit 5
fi
git merge --no-ff $CURRENT_BRANCH -m "Merge $CURRENT_BRANCH into $PROD_BRANCH"
if [ "$?" != "0" ]; then
    echo "git merge --no-ff $CURRENT_BRANCH -m \"Merge $CURRENT_BRANCH into $PROD_BRANCH\":  Failed (with code $?), exiting"
    exit 5
fi
